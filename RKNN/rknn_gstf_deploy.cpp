...
int main(int argc, char *argv[])
{
    ...
    // 分配内存并生成随机数据
    float* data = (float*)malloc(BATCH_SIZE * DIM1 * DIM2 * sizeof(float));
    if (data == NULL) {
    ...
    input[0].index = 0;
    input[0].buf = data;
    input[0].size = BATCH_SIZE * DIM1 * DIM2;
    ...
    // 打印输出数据
    printf("Output0 tensor size: %d\n", output[0].size);
    printf("Output1 tensor size: %d\n", output[1].size);
    // float* buffer = (float*)output[0].buf;
    // for (int i = 0; i < 10; i++) {
    //     printf("%f ", buffer[0]);
    // }
    // printf("\n");
