...
########################################################################################################################
'''
Modified by Yuxiang Hua, CASIC-2-206, 2024-10-28
...
class XGBDIM():
    ...
    def __init__(self, data_path, sub_idx, trainset, validationset,
                 model_path, model_load_path, X1_input, X2_input,
        ...
        self.data_path = data_path  ## only used in def test()
    ...
    def get_data_ZT206_HYX_train(self):
        X1 = self.X1_input
        X2 = self.X2_input
        K1 = np.shape(X1)[2]
        K2 = np.shape(X2)[2]
    ...
    def get_data_ZT206_HYX_valid(self):
        X1 = self.X1_input
        X2 = self.X2_input
        K1 = np.shape(X1)[2]
        K2 = np.shape(X2)[2]
    ...
    def load_model(self, ):
        ...
        model = np.load(self.model_load_path)
    ...
    def get_3D_cuboids_ZT206_HYX(self, X1, K1):
        Tset = np.zeros((K1, self.T_local, self.N_chanwin * self.N_win))
        idx_conv = -1
        for idx_chan in range(self.N_chanwin):
            for idx_win in range(self.N_win):
    ...
    def preprocess_ZT206_HYX(self, X1, K1):
        #X1 = np.delete(X1, list(range(self.n_cutpoint)), axis=1)
        for k in range(K1):
            X1[:, :, k] = X1[:, :, k] - np.mean(X1[:, :, k], axis=1)[:, np.newaxis]
        # print('EEG processed !')
    ...
    def predict_ZT206_HYX(self, X1):

        self.get_3Dconv()

        K1t = X1.shape[2]
        X1 = self.preprocess_ZT206_HYX(X1, K1t)
        Tset_test = self.get_3D_cuboids_ZT206_HYX(X1, K1t)

        Tset_test_global = X1.copy()
        X_test = Tset_test
        X_test_global = Tset_test_global
        label_test = np.ones((K1t, 1))

        # X_test = np.concatenate((Tset_test, Tset_test), axis=0)
        # X_test_global = np.concatenate((Tset_test_global, Tset_test_global), axis=2)
        # label_test = np.concatenate((np.ones((K1t, 1)), np.zeros((K1t, 1))), axis=0)
    ...
    def train_model(self):
        ...
        filename = 'Model_' + str(self.sub_idx) + '-' + str(current_time) + '.npz'
...
def ZT206_HYX_prog_CPU(predict_BCIdataset_path, predict_BCImodel_path, train_label):
    model_save_path = './'  ## pt文件保存路径
    sub_ID = 1

    data = np.load(predict_BCIdataset_path)
    X1 = data['X1']   ## positive samples
    X2 = data['X2']   ## negative samples

    mdoel_load_path = predict_BCImodel_path

    xgb = XGBDIM('./', sub_ID, np.array([1]), np.array([2, 3, 4]),
                 model_save_path, mdoel_load_path, X1, X2,
                 50, 6, 3, 3, 3, 3,
                 0.5, 0.05, 100, 20, 1, 1, 299, 0.3,
                 True, 30, False, False)
    ...
    if train_label==True:
        ...
        while i < 50:
            t1 = time.time()
            ba, acc, tpr, fpr, auc, y_predicted_final = xgb.predict_ZT206_HYX(X1_test)
            t2 = time.time()
            print("cost :", t2 - t1)
            print('ba, acc, tpr, fpr, auc, y_predicted_final == ', ba, acc, tpr, fpr, auc, y_predicted_final[0][0])
            i = i + 1
########################################################################################################################

if __name__ == "__main__":
    ZT206_HYX_prog_CPU('./traindata_CJH.npz', './Model_1-20241104-165633.npz', train_label=False)